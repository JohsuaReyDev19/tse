...
if ((isset($_POST["POSTcheck"])) && ($_POST["POSTcheck"] == "form1")) {
  ...
  $events_status = ($submittedDate >= $currentDate) ? "active" : "disabled";

  // Insert Event
  $SQLcrud = "INSERT INTO events (`events_description`, events_date, events_status) VALUES (?,?,?)";
  $db->query($SQLcrud);
  ...
  // File Upload Handling (JPG only)
  $filenames = "../images/events/" . $lastID . ".jpg";

  if (isset($_FILES['image-upload1']) && $_FILES['image-upload1']['error'] === 0) {
    move_uploaded_file($_FILES['image-upload1']['tmp_name'], $filenames);
  }

  // Redirect to Category List for this new event
  $GoTo = "../category/category_list.php?recordID=" . $lastID;
          ...
          <fieldset>
            ...
            <div class="row">
              <div class="form-group col-md-8 col-sm-12">
                ...
                <input required type="text" class="form-control" name="events_description" id="events_description"
                  placeholder=" "
                  value="<?php if (isset($_POST['events_description'])) echo $_POST['events_description']; ?>"
                  onkeyup="showAjax('events_duplicate.php','txtString',this.value, 'results_here');"
                  onblur="showAjax('events_duplicate.php','txtString',this.value, 'results_here');">

              </div>

              <div class="form-group col-md-4 col-sm-12">
                <label for="events_date">Date</label>
                <input required type="date" min="<?php echo date('Y-m-d'); ?>" class="form-control"
                  name="events_date" id="events_date"
                  value="<?php if (isset($_POST['events_date'])) echo $_POST['events_date']; ?>">
              </div>
            </div>
            ...
            <div class="form-group">
              ...
              <div class="col-md-10">
                ...
                <a href="events_list.php" class="btn btn-outline-danger hidelink">
                  <span class="bi-x-octagon"></span> Cancel
                </a>
              </div>
            </div>
