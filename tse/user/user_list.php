...
$query_rs = "SELECT u.* FROM `user` u WHERE status!='pending approval' order by u.`fullname`";
$db->query($query_rs);
$rs=$db->rowset();
...
<?php if ($rspending_total>0) {?>
    <div class="row"><br></div>
    <legend><span class="text-danger">List of User with Pending Approval</span></legend>
<table id="tablelist1" class="table table-hover table-responsive table-bordered" >
        <thead >
...
<?php foreach ($rspending as $rs_rspending){ ?>
    <tr>
    <td class="align-middle"><?php echo htmlentities($rs_rspending['fullname']); ?></td>
    <td class="align-middle"><?php echo htmlentities($rs_rspending['designation']); ?></td>
    <td class="align-middle"><?php echo htmlentities($rs_rspending['group']); ?></td>
    <td class="align-middle"><?php echo htmlentities($rs_rspending['status']); ?></td>
    <td  class="align-middle" width="110px">
        <div class="btn-group" role="group" align="center">
           
            <a href='user_update.php?recordID=<?php echo $rs_rspending["id"];?>' class="btn btn-outline-success" class="btn btn-primary " data-toogle="tooltip" data-placement="bottom" title="Update"><span class="bi-person-check-fill" ></span></a>
...
<?php foreach ($rs as $rs_user){ ?>
    ...
    <td  class="align-middle" width="110px">
        <div class="btn-group" role="group" align="center">
            ...
            <a href='user_update.php?recordID=<?php echo $rs_user["id"];?>' class="btn btn-outline-success" class="btn btn-primary " data-toogle="tooltip" data-placement="bottom" title="Update"><span class="bi-person-check-fill" ></span></a>
             <?php  if (strcmp(htmlentities($rs_user['group']),"Administrator") && strcmp(htmlentities($rs_user['group']),"Judge")) { ?>
            <a href='user_remove.php?recordID=<?php echo $rs_user["id"];?>' class="btn btn-outline-danger" data-toogle="tooltip"  data-placement="bottom" title="Remove"><span class="bi-person-dash-fill" ></span></a>
            <?php } ?>        
        </div>
    </td>
    </tr>
...
<script>
    ...
    const dataTable2 = new simpleDatatables.DataTable("#tablelist1", {
        searchable: true,
        fixedHeight: true,
        //perPage: 25,
        //labels: labelData,
