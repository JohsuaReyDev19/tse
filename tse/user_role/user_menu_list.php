...
<?php foreach ($rs as $rs_data){ ?>
    ...
    <td class="align-middle"><?php if ((strcmp($rs_data['id'],$rs_data['parent_id']))) { echo '&nbsp;&nbsp;&nbsp;<span class="bi bi-arrow-right-short"></span>&nbsp;'; echo '<a href="user_menu_details.php?id='.htmlentities($rs_data['id']).'&menu='.htmlentities($rs_data['name']).'">'.htmlentities($rs_data['name']).'</a>';} else { echo '<a href="user_menu_details.php?id='.htmlentities($rs_data['id']).'&menu='.htmlentities($rs_data['name']).'">'.htmlentities($rs_data['name']).'</a>';} ?></td>
...
?>
  <?php if ($totalRows_rssub>0) { foreach ($rssub as $rssub_data){ ?>
    ...
    <tr>
    <td class="align-middle"><?php if ((strcmp($rssub_data['id'],$rssub_data['parent_id']))) { echo '&nbsp;&nbsp;&nbsp;<span class="bi bi-arrow-right-short"></span>&nbsp;'; echo '<a href="user_menu_details.php?id='.htmlentities($rssub_data['id']).'&menu='.htmlentities($rssub_data['name']).'">'.htmlentities($rssub_data['name']).'</a>';} else { echo '<a href="user_menu_details.php?id='.htmlentities($rssub_data['id']).'&menu='.htmlentities($rssub_data['name']).'">'.htmlentities($rssub_data['name']).'</a>';} ?></td>
