...
<div class="card">
        <div class="card-header"><h5 class="card-title"><strong><?php echo 'User Restriction for <strong>'.$_GET['id'].'</strong>';?></strong></h5></div>
            <div class="card-body">
<!--------------------------------------------------------------------------------->
...
<?php foreach ($rs as $rs_data){ ?>
    ...
    <td class="align-middle"><input type="checkbox" style="vertical-align: middle; margin: 0px;" name="<?php echo htmlentities($rs_data['id']); ?>" id="<?php echo htmlentities($rs_data['id']); ?>" value="<?php echo htmlentities($rs_data['id']); ?>" 
     onclick="sendID('user_group_restriction_updater.php','menu_id',this.value + '&group=<?php echo $_GET['id']; ?>');"
    <?php
      echo $phu->find_restriction($_GET['id'],htmlentities($rs_data['id']));
    ?>
    ></td>
    <td class="align-middle"><?php if ((strcmp($rs_data['id'],$rs_data['parent_id']))) { echo '&nbsp;&nbsp;&nbsp;<span class="bi bi-arrow-right-short"></span>&nbsp;'; echo '<a href="user_group_restriction_details.php?id='.htmlentities($rs_data['id']).'&menu='.htmlentities($rs_data['name']).'&group='.$_GET['id'].'">'.htmlentities($rs_data['name']).'</a>';} else { echo '<a href="user_group_restriction_details.php?id='.htmlentities($rs_data['id']).'&menu='.htmlentities($rs_data['name']).'&group='.$_GET['id'].'">'.htmlentities($rs_data['name']).'</a>';} ?></td>
  ...
  <?php if ($totalRows_rssub>0) { foreach ($rssub as $rssub_data){ ?>
   ...
   <tr>
    <td data-sortable="false" class="align-middle"><input type="checkbox" style="vertical-align: middle; margin: 0px;" name="<?php echo htmlentities($rssub_data['id']); ?>" id="<?php echo htmlentities($rssub_data['id']); ?>" value="<?php echo htmlentities($rssub_data['id']); ?>" 
     onclick="sendID('user_group_restriction_updater.php','menu_id',this.value + '&group=<?php echo $_GET['id']; ?>');"
    <?php
      echo $phu->find_restriction($_GET['id'],htmlentities($rssub_data['id']));
    ?>
    ></td>
    <td data-sortable="false" class="align-middle"><?php if ((strcmp($rssub_data['id'],$rssub_data['parent_id']))) { echo '&nbsp;&nbsp;&nbsp;<span class="bi bi-arrow-right-short"></span>&nbsp;'; echo '<a href="user_group_restriction_details.php?id='.htmlentities($rssub_data['id']).'&menu='.htmlentities($rssub_data['name']).'&group='.$_GET['id'].'">'.htmlentities($rssub_data['name']).'</a>';} else { echo '<a href="user_group_restriction_details.php?id='.htmlentities($rssub_data['id']).'&menu='.htmlentities($rssub_data['name']).'&group='.$_GET['id'].'">'.htmlentities($rssub_data['name']).'</a>';} ?></td>
